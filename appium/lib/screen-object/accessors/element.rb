...
module ScreenObject
  module AppElements
    ...
    class Element
      ...
      def scroll_element_to_view(direction = :down, time_out = 30, xtr_scroll = false)
        wait_until(time_out,'Unable to find element',&->{element_visible?(direction, xtr_scroll)})
      end

      # method for scrolling until element is visible and click.
      # this will NOT return any value.
      # @param [direction] 'Down', 'up'
      def scroll_element_to_view_click(direction= :down, time_out = 40, xtr_scroll = false)
        wait_until(time_out,'Unable to find element',&->{element_visible?(direction, xtr_scroll)})
        click
      end
      ...
      def get_children(identifier)
        element.find_elements(identifier)
      end

      def get_text_children
        text_class = driver.device_is_android? ? "" : ""
        element.find_elements(class:"#{}")
      end
    end
  end
end
