...
module ScreenObject
 included AppElements
  ...
  def scroll_text_to_view(text, direction = :down, timeout = 40)
    wait_until(timeout,"Unable to find element text: #{text}",&->{text_visible?(text, direction)})
  end
  ...
  # @param direction [symbol] The direction to search for an string
  # @return          [Boolean]
  def text_visible?(text, direction = :down, xtr_scroll = false)
  ...
  def click_exact_text(text)
    ScreenObject::Element.get_element_by_exact_text(text).click
  end
  ...
  def wait_for_text(timeout = 30, text)
    wait_until(timeout,"text <<#{text} is not visible")
  end
end
